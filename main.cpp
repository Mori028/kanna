...
//デバッグレイヤーをオンに
	...
	while (true) {
		...
		//ブーメラン横幅調整用変数
		float PlayerBulletWidth = 1.0f;

		// ----- ベクトル ----- //
		/*XMFLOAT3 frontV = { eye.x - target.x,eye.y - target.y,eye.z - target.z };

		XMFLOAT3 sideV = {0, 0, -PlayerBulletWidth * frontV.x };*/
	...
	/*	targetSideTranslation = { -sideV.x, target.y + 3,-sideV.z };
		...
		if (sceneNo_ == SceneNo::Title) {
			...
			PlayerBulletPosition = { 0.0f,0.0f,0.0f };
			PlayerBulletScale = { 1.0f,1.0f,1.0f };
		...
		if (sceneNo_ == SceneNo::Game) {
			...
			if (bezierMode == TRUE) {
				...
				PlayerBulletPosition.z++;
				if (timer >= splitNum) {
					PlayerBulletPosition.z = 0;
					timer = 0;
					bezierMode = FALSE;
				}
			}
		...
		if (sceneNo_ == SceneNo::Clear) {
			...
			if (input->TriggerKey(DIK_E) && sceneNo_ == SceneNo::Clear) {
